...
- (BOOL) setupMetal:(kContrastKernelFunction)kContrastKernelFunction
{
    NSError *error = NULL;
    ...
    id<MTLFunction> contrastKernelFunction = [defaultLibrary newFunctionWithName:(kContrastKernelFunction == kContrastKernelFunctionReduction)
                                              ?
                                              [NSString stringWithFormat:@"contrastReductionKernel"]
                                              :
                                              [NSString stringWithFormat:@"contrastStretchKernel"]];
    
    // Create a compute pipeline state
    self.computePipelineState = [self.device newComputePipelineStateWithFunction:contrastKernelFunction
                                                                           error:&error];
...
+ (BOOL)processWithInputs:(NSArray<id<CIImageProcessorInput>> *)inputs arguments:(NSDictionary<NSString *,id> *)arguments output:(id<CIImageProcessorOutput>)output error:(NSError * _Nullable *)error
{
    ...
    BOOL worked = [metalKernel setupMetal:kContrastKernelFunctionStretch];
